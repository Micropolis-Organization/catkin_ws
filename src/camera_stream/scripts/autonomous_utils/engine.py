...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, boxes, confs, clss, depths=None):
        ...
        if depths == None:
            depths = ["" for i in clss]

        for bb, cf, cl, depth_value in zip(boxes, confs, clss, depths):
            cl = int(cl)
            x_min, y_min, x_max, y_max = bb[0], bb[1], bb[2], bb[3]
            color = self.colors[cl]
            cv2.rectangle(img, (x_min, y_min), (x_max, y_max), color, 2)
            # cv2.rectangle(blk, (x_min, y_min), (x_max, y_max), color, cv2.FILLED)
            cls_name = self.cls_dict.get(cl, 'CLS{}'.format(cl)) + ", Depth: " + str(depth_value)
